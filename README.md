...
### 7. Proceso de compra
Para realizar el proceso de compra, he creado la función realizar_compra (), la lista carrito vacía para ir añadiendo los productos, el valor compra_final para ir haciendo la suma de precios acumulados del carrito y la variable aniadir_carrito= "sí" para que se inicie directamente el while. La estructura sería la siguiente:
1. Se imprime el inventario para ver los productos (aquí tengo que decir que no me gusta cómo se ve, me hubiera gustado hacerlo de otra forma para que la impresión se viera más clara)
2. Se pide al usuario que escriba el nombre del producto que quiere comprar y la cantidad (esta convierte el string a int).
3. Cuando el producto indicado está en el inventario:
    - Se comprueba que la cantidad indicada esté disponible, si lo está:
        - Se añade al carrito y se multiplica la cantidad por el precio
        - Se actualiza el inventario
        - Se imprime un resumen de la compra
        - Se pregunta si se desea seguir comprando (while), si la respuesta es sí, se repite el proceso.
            Si la respuesta es no, se imprime el precio final, se indica que la compra ha finalizado y se imprime el inventario actualizado.
    - Si la cantidad no está disponible, se indica que solo quedan X unidades, y se vuelve a preguntar si quiere seguir comprando, para ver si se repite el while o se finaliza la compra.
4. Cuando el producto no está en el inventario (el else dentro del for), se indica.
